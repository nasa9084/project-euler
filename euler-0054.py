...
# RoyalFlush = 9
def getHand(cards):
    nums = [cards[i][0] for i in range(5)]
    for i in range(5):
        if nums[i]=="T":
            nums[i]=10
        elif nums[i]=="J":
    ...
    suits = [cards[i][1] for i in range(5)]
    for i in range(4):
        ...
        straight = True
    if len(set(suits))==1:
        if (10 in nums) and (11 in nums) and (12 in nums) and (13 in nums) and (14 in nums):
            return (9, nums)
        elif straight:
            return (8, nums)
        flush = True
    ...
    if nums.count(nums[0])==4:
        return (7, [nums[0]])
    if nums.count(nums[4])==4:
        return (7, [nums[4]])
    if len(set(nums))==2:
        return (6, list(set(nums)))
    if flush:
        return (5, nums)
    if straight:
        return (4, nums)
    ...
    if len(set(nums))==3:
        for i in range(3):
            if nums.count(nums[i])==1:
                break
        return (2, list(set([n for j, n in enumerate(nums) if j!=i])))
    for i in range(4):
        if nums.count(nums[i])==2:
            return (1, [nums[i]])
    return (0, nums)

def compCards(sortedCards1, sortedCards2):
    if not sortedCards1 or not sortedCards2:
        return 0
    last1 = sortedCards1[-1]
    last2 = sortedCards2[-1]
    if last1>last2:
        return 1
    elif last1==last2:
        return compCards(sortedCards1[:-1], sortedCards2[:-1])
    else:
        return -1

patterns = """8C TS KC 9H 4S 7D 2S 5D 3S AC
...
for pattern in patterns:
    ...
    player1 = getHand(pattern[:5])
    player2 = getHand(pattern[5:])
    if player1[0]>player2[0] or (player1[0]==player2[0] and compCards(player1[1],player2[1])>0):
        count += 1
